 ...
 *
 * 如果第一个非空字符为正或者负号时，则将该符号与之后面尽可能多的连续数字字符组合起来，形成一个有符号整数。 假如第一个非空字符是数字，则直接将其与之后连续的数字字符组合起来，形成一个整数。
 ...
 * 提示：
 *
 * 本题中的空白字符只包括空格字符 ' ' 。 假设我们的环境只能存储 32 位大小的有符号整数，那么其数值范围为 [−231,  231 − 1]。如果数值超过这个范围，请返回  INT_MAX
 * (231 − 1) 或 INT_MIN (−231) 。
 *
 *
 * 示例 1:
...
public class Solution6 {
  ...
  class Automaton {
    ...
    Automaton() {
      ...
      number = 0;
      map.put(Status.START, new Status[]{Status.START, Status.SINGED, Status.NUM, Status.END});
      map.put(Status.SINGED, new Status[]{Status.END, Status.END, Status.NUM, Status.END});
      map.put(Status.NUM, new Status[]{Status.END, Status.END, Status.NUM, Status.END});
      map.put(Status.END, new Status[]{Status.END, Status.END, Status.END, Status.END});
    }

    public void get(char c) {
      status = map.get(status)[getCol(c)];
      if (status == status.SINGED) {
        if (Objects.equals(c, '-')) {
          flag = -1;
          max = max + 1;
        }
  ...
  public int myAtoi1(String x) {
    Automaton automaton = new Automaton();
    for (char c : x.toCharArray()) {
      automaton.get(c);
    }
    return ((int) automaton.number) * automaton.flag;
  }
  ...
  public int myAtoi(String str) {
    ...
    return (int) re * (flag ? 1 : -1);
  }

  public static void main(String[] args) {
    Solution6 solution3 = new Solution6();
    int ss = solution3.myAtoi("+-2");
